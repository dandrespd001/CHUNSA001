...
int main() {
    ...
    static constexpr uint8_t kExpectedHash[32] = {
        0xd6, 0xe0, 0xf8, 0x29, 0x45, 0x46, 0x2b, 0xd2, 0x02, 0x12, 0xa6, 0x16, 0xbe, 0x50, 0x4f, 0xba,
        0x75, 0x1e, 0x1b, 0xbf, 0xb8, 0x71, 0x1c, 0xb8, 0x3f, 0x5a, 0x07, 0x49, 0xd3, 0x31, 0x91, 0xc4,
    };
    CHECK(std::memcmp(cat.content_hash.bytes, kExpectedHash, 32) == 0);
    CHECK(cat.blob_format_major == 1 && cat.blob_format_minor == 2);
    CHECK(cat.schema_set_version == 3);
