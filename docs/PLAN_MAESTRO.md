...
#### Sprint 1.3 — UI/HUD v1 ✅ COMPLETO (2026-07-24, ver §1 y REPORTE_SPRINT_1.3)
Desviación del plan: lo hizo Codex/Luna Max (Kimi sigue sin cuota), 100% frontend, sin tocar
kernel. Decisión de diseño (SPEC-006 §5): los números `1..9` pasaron a grupos de control y la
producción del 1.2 migró de teclas numéricas a botones. Fog of war diferido (deuda 1.4/1.5).
...
| **1.8B** ✅ | **SPEC-007 §18** · concordancia C5–C7 | **Reconciliación del esquema**: eliminar el enum de 8 letras, fusionar `material_costs` en `resource_costs`, sacar `El` del stock. Recursos por `record_id`; el compilador asigna índices. Renombrar A/B/Me → comida/madera/piedra **conservando 0,1,2** | Blob reproducible · `end_tick` **idénticos** · un id inexistente es error de carga · sin recursos nuevos aún |
| **1.8C** 🔶 | SPEC-007 §9.2 · SPEC-006 Parte III | **Definiciones**: los 30 recursos en `data/resources/` con familia y edad · **HUD por familias**. Sin tocar mapa ni costes | El HUD muestra nombres y familias reales · `end_tick` **idénticos**: es solo vocabulario |
...
| **1.20** ✅ | **SPEC-008** | **Banco de rendimiento**: mide `Step()` y `state_checksum_v1` en el escenario de §2.1 y falla ante regresión frente a una referencia registrada. Etiqueta `perf`, **fuera** del ciclo de PR: el tiempo de pared no es determinista y una prueba que falla sola se acaba ignorando | Entregado, y **encontró algo en su primera ejecución**. Primeras medidas reales del proyecto: `Step()` **970 µs** (dentro del presupuesto, margen ×2,1 y no ×25) y **checksum 532 µs, que SUPERA en ×2,7 su presupuesto de 0,2 ms**. Causa localizada: hashea `cost_grid` **byte a byte, 65 536 llamadas por tick** |
| **1.21** (nuevo) | SPEC-008 | **Abaratar el checksum**: hashear `cost_grid` en palabras de 64 bits en vez de byte a byte — 8 192 llamadas en lugar de 65 536, misma información | Sale del 1.20 con número medido. `cost_grid` **es estado real** —los edificios escriben muro al construirse (`step.hpp:443`) y lo limpian al morir— así que no se puede excluir del dominio, solo hashear mejor. Implica **bump de checksum** y re-registro de baselines, por eso va en su propio sprint y no al final de otro |
